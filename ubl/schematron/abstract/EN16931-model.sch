...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron" abstract="true" id="model">
  ...
  <rule context="$Document_totals ">
    ...
    <assert test="$BR-CO-13" flag="fatal" id="BR-CO-13">[BR-CO-13]-Invoice total amount without VAT [<value-of select="f:comoNumero(cbc:TaxExclusiveAmount, 0)" />] (BT-109) = [<value-of select="f:comoNumero(cbc:LineExtensionAmount,0) + f:comoNumero(cbc:ChargeTotalAmount,0) - f:comoNumero(cbc:AllowanceTotalAmount,0)"/>] Σ Invoice line net amount (BT-131) [<value-of select="f:comoNumero(cbc:LineExtensionAmount, 0)" />] - Sum of allowances on document level (BT-107) [<value-of select="f:comoNumero(cbc:AllowanceTotalAmount, 0)" />] + Sum of charges on document level (BT-108) [<value-of select="f:comoNumero(cbc:ChargeTotalAmount, 0)" />].</assert>
    <assert test="$BR-CO-16" flag="fatal" id="BR-CO-16">[BR-CO-16]-Amount due for payment (BT-115) = Invoice total amount with VAT (BT-112) - Paid amount (BT-113) + Sum of amounts collected in the name of a third party (BT-179) + Rounding amount (BT-114).</assert>
