...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron" is-a="model" id="UBL-model">
  ...
  <param name="BR-CO-14" value="
    not(cac:TaxSubtotal) or 
    xs:decimal(child::cbc:TaxAmount) = f:redondeaImporte(sum(cac:TaxSubtotal/xs:decimal(cbc:TaxAmount)))"/>
  
  
  <!-- BR-CO-15 reescrita para que se entienda mejor -->
  <param name="BR-CO-15" value="
    every $Currency in cbc:DocumentCurrencyCode satisfies (
      count(cac:TaxTotal/xs:decimal(cbc:TaxAmount[@currencyID=$Currency])) eq 1) and 
  ...
  <param name="BR-CO-32" value="f:comoNumero(cbc:LineExtensionAmount, 0) = 
    f:redondeaImporte(
      (f:comoNumero(cac:Price/cbc:PriceAmount, 0) div f:comoNumero(cac:Price/cbc:BaseQuantity, 1)) * f:comoNumero(cbc:InvoicedQuantity, 1)) + 
    ...
    <!-- Nueva regla EN-16931:2026 -->
    <param name="BR-CO-38" value="
      ./cbc:ChargeIndicator = true() or 
      (not(exists(cbc:BaseAmount)) and not (exists(cbc:MultiplierFactorNumeric))) or 
      f:comoNumero(cbc:Amount, 0) = f:redondeaImporte((f:comoNumero(cbc:BaseAmount, 0) * f:comoNumero(cbc:MultiplierFactorNumeric, 0)))
    " />  
    
    <!-- Nueva regla EN-16931:2026 -->
    <param name="BR-CO-39" value="
      ./cbc:ChargeIndicator = false() or 
      (not(exists(cbc:BaseAmount)) and not (exists(cbc:MultiplierFactorNumeric))) or 
      f:comoNumero(cbc:Amount, 0) = f:redondeaImporte((f:comoNumero(cbc:BaseAmount, 0) * f:comoNumero(cbc:MultiplierFactorNumeric, 0)))
    " />  
...
<param name="BR-S-01" value="
      ...
      and 
      (count(//cac:AllowanceCharge/cac:TaxCategory[normalize-space(cbc:ID) = 'S']) + count(//cac:ClassifiedTaxCategory[normalize-space(cbc:ID) = 'S']))  &gt; 0) 
    or 
      (count(cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory[normalize-space(cbc:ID) = 'S']) = 0 
      and 
      (count(//cac:AllowanceCharge/cac:TaxCategory[normalize-space(cbc:ID) = 'S']) + count(//cac:ClassifiedTaxCategory[normalize-space(cbc:ID) = 'S'])) = 0)
 ...
 <param name="BR-S-03" value="
  ...
  <!-- BR-S-08 Mejora importante en la forma de calcular márgenes de error y agrupar información (Invoice vs CreditNote) -->
  <param name="BR-S-08" value="
    every $rate in xs:decimal(cbc:Percent) satisfies (
        (
          exists(//cac:InvoiceLine[cac:Item/cac:ClassifiedTaxCategory/normalize-space(cbc:ID) = 'S'][cac:Item/cac:ClassifiedTaxCategory/xs:decimal(cbc:Percent) = $rate]) 
        or 
          exists(//cac:CreditNoteLine[cac:Item/cac:ClassifiedTaxCategory/normalize-space(cbc:ID) = 'S'][cac:Item/cac:ClassifiedTaxCategory/xs:decimal(cbc:Percent) = $rate]) 
        or 
          exists(//cac:AllowanceCharge[cac:TaxCategory/normalize-space(cbc:ID)='S'][cac:TaxCategory/xs:decimal(cbc:Percent) = $rate])
        ) 
        and 
          f:enMargen(../f:comoNumero(cbc:TaxableAmount, 0), 
            f:comoNumero(../../../cac:InvoiceLine[cac:Item/cac:ClassifiedTaxCategory/normalize-space(cbc:ID)='S'][cac:Item/cac:ClassifiedTaxCategory/xs:decimal(cbc:Percent) = $rate]/xs:decimal(cbc:LineExtensionAmount), 0) 
            +
            f:comoNumero(../../../cac:CreditNoteLine[cac:Item/cac:ClassifiedTaxCategory/normalize-space(cbc:ID)='S'][cac:Item/cac:ClassifiedTaxCategory/xs:decimal(cbc:Percent) = $rate]/xs:decimal(cbc:LineExtensionAmount), 0) 
            + 
            f:comoNumero(../../../cac:AllowanceCharge[cbc:ChargeIndicator=true()][cac:TaxCategory/normalize-space(cbc:ID)='S'][cac:TaxCategory/xs:decimal(cbc:Percent) = $rate]/xs:decimal(cbc:Amount), 0) 
            - 
            f:comoNumero(../../../cac:AllowanceCharge[cbc:ChargeIndicator=false()][cac:TaxCategory/normalize-space(cbc:ID)='S'][cac:TaxCategory/xs:decimal(cbc:Percent) = $rate]/xs:decimal(cbc:Amount), 0)
          , 1))
  "/>

  <!-- BR-S-09 Mejora importante en la forma de calcular márgenes de error -->
  <param name="BR-S-09" value="
    f:enMargen(
      f:comoNumero(../cbc:TaxAmount, 0), 
      f:redondeaImporte(f:comoNumero(../cbc:TaxableAmount, 0) * (f:comoNumero(cbc:Percent, 0) div 100)),
      1) 
  "/>

  <!-- BR-S-10: Suprimida en versión de 2026 -->
  ...
  <!-- BR-Z-10: Suprimida en versión de 2026 -->
  ...
  <!-- BR-O-2..5: Suprimidas en versión de 2026 -->
  ...
  <param name="BR-AF-09" value="(abs(xs:decimal(../cbc:TaxAmount)) - 1 &lt;  round((abs(xs:decimal(../cbc:TaxableAmount)) * (xs:decimal(cbc:Percent) div 100)) * 10 * 10) div 100 ) and (abs(xs:decimal(../cbc:TaxAmount)) + 1 &gt;  round((abs(xs:decimal(../cbc:TaxableAmount)) * (xs:decimal(cbc:Percent) div 100)) * 10 * 10) div 100 )"/>
  <param name="BR-AF-10" value="not(cbc:TaxExemptionReason) and not(cbc:TaxExemptionReasonCode)"/>
  <param name="BR-AG-01" value="((count(//cac:AllowanceCharge/cac:TaxCategory[normalize-space(cbc:ID) = 'M'][cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']) + count(//cac:ClassifiedTaxCategory[normalize-space(cbc:ID) = 'M'][cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT'])) &gt; 0 and count(cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory[cbc:ID = 'M'][cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']) &gt; 0) or ((count(//cac:AllowanceCharge/cac:TaxCategory[normalize-space(cbc:ID) = 'M'][cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']) + count(//cac:ClassifiedTaxCategory[normalize-space(cbc:ID) = 'M'][cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT'])) = 0 and count(cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory[normalize-space(cbc:ID) = 'M'][cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']) = 0)"/>
  <param name="BR-AG-02" value="(exists(//cac:ClassifiedTaxCategory[normalize-space(cbc:ID) = 'M'][cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']) and (exists(//cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme/cbc:CompanyID) or exists(//cac:TaxRepresentativeParty/cac:PartyTaxScheme[cac:TaxScheme/(normalize-space(upper-case(cbc:ID)) = 'VAT')]/cbc:CompanyID))) or not(exists(//cac:ClassifiedTaxCategory[normalize-space(cbc:ID) = 'M'][cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']))"/>
  <param name="BR-AG-03" value="(exists(//cac:AllowanceCharge[cbc:ChargeIndicator=false()]/cac:TaxCategory[normalize-space(cbc:ID)='M'][cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']) and (exists(//cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme/cbc:CompanyID) or exists(//cac:TaxRepresentativeParty/cac:PartyTaxScheme[cac:TaxScheme/(normalize-space(upper-case(cbc:ID)) = 'VAT')]/cbc:CompanyID))) or not(exists(//cac:AllowanceCharge[cbc:ChargeIndicator=false()]/cac:TaxCategory[normalize-space(cbc:ID)='M'][cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']))"/>
  <param name="BR-AG-04" value="(exists(//cac:AllowanceCharge[cbc:ChargeIndicator=true()]/cac:TaxCategory[normalize-space(cbc:ID)='M'][cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']) and (exists(//cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme/cbc:CompanyID) or exists(//cac:TaxRepresentativeParty/cac:PartyTaxScheme[cac:TaxScheme/(normalize-space(upper-case(cbc:ID)) = 'VAT')]/cbc:CompanyID))) or not(exists(//cac:AllowanceCharge[cbc:ChargeIndicator=true()]/cac:TaxCategory[normalize-space(cbc:ID)='M'][cac:TaxScheme/normalize-space(upper-case(cbc:ID))='VAT']))"/>
  <param name="BR-AG-05" value="(cbc:Percent) &gt;= 0"/>
  <param name="BR-AG-06" value="(cbc:Percent) &gt;= 0"/>
  <param name="BR-AG-07" value="(cbc:Percent) &gt;= 0"/>
  ...
  <param name="BR-AG-09" value="(abs(xs:decimal(../cbc:TaxAmount)) - 1 &lt;  round((abs(xs:decimal(../cbc:TaxableAmount)) * (xs:decimal(cbc:Percent) div 100)) * 10 * 10) div 100 ) and (abs(xs:decimal(../cbc:TaxAmount)) + 1 &gt;  round((abs(xs:decimal(../cbc:TaxableAmount)) * (xs:decimal(cbc:Percent) div 100)) * 10 * 10) div 100 )"/>
  <param name="BR-AG-10" value="not(cbc:TaxExemptionReason) and not(cbc:TaxExemptionReasonCode)"/>
